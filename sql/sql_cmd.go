...
func initCommand(db *sqlx.DB, connName string, log log.Interface, isLeader, logQuery bool) Command {
	c := &command{
		db:       db,
		connName: connName,
...
func (c *command) Prepare(ctx context.Context, name string, query string) (CommandStmt, error) {
	...
	return initStmt(ctx, name, c.connName, stmt, c.connType == connTypeLeader), nil
}
...
func (c *command) BeginTx(ctx context.Context, name string, opt TxOptions) (CommandTx, error) {
	...
	return initTx(ctx, name, c.connName, tx, opts, c.log, c.connType == connTypeLeader, c.logQuery), nil
}
