...
func initTx(ctx context.Context, name, connName string, tx *sqlx.Tx, opts *sql.TxOptions, log log.Interface, isLeader, logQuery bool) CommandTx {
	c := &commandTx{
		ctx:      ctx,
		name:     name,
		connName: connName,
...
func (x *commandTx) Prepare(name string, query string) (CommandStmt, error) {
	...
	return initStmt(x.ctx, name, x.connName, stmt, x.connType == connTypeLeader), nil
}
...
func (x *commandTx) Stmt(name string, stmt *sqlx.Stmt) CommandStmt {
	...
	// 	defer timer.ObserveDuration()
	// }
	return initStmt(x.ctx, name, x.connName, stmt, x.connType == connTypeLeader)
}
