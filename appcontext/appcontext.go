...
const (
	...
	deviceToken      contextKey = "DeviceToken"
)
...
func SetDeviceToken(ctx context.Context, val string) context.Context {
	return context.WithValue(ctx, deviceToken, val)
}

func GetDeviceToken(ctx context.Context) string {
	val, ok := ctx.Value(deviceToken).(string)
	if !ok {
		return ""
	}
	return val
